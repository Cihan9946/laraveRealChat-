...
class SocketController extends Controller implements MessageComponentInterface
{
    ...
    {
        // Yeni bağlantıyı bağlı istemciler listesine ekleyin
        $this->clients->attach($conn);

        // Bağlantı üzerinden gelen sorgu dizesini alın
        $querystring = $conn->httpRequest->getUri()->getQuery();

        // Sorgu dizesini diziye dönüştürün
        parse_str($querystring, $queryarray);

        // Eğer bağlantıda 'token' varsa
        if (isset($queryarray['token'])) {
            // Kullanıcının bağlantı kimliğini ve durumunu güncelleyin (Online olarak işaretle)
            User::where('token', $queryarray['token'])->update(['connection_id' => $conn->resourceId, 'user_status' => 'Online']);

            // Kullanıcının kimliğini seçin
            $user_id = User::select('id')->where('token', $queryarray['token'])->get();
            $data['id'] = $user_id[0]->id;

            // Kullanıcının durumunu 'Online' olarak ayarlayın
            $data['status'] = 'Online';

            // Bağlı olan diğer istemcilere kullanıcının durumunu gönder
    ...
    public function onMessage(ConnectionInterface $conn, $msg)
    {
        ...
        if(isset($data->type))
        {
            ...
            if($data->type == 'request_send_message')
            {
                ...
                /*$temsilciler = User::where('birim', $data->birim_id)->where('status', 'online')->pluck('id')->toArray();

                foreach ($temsilciler as $t) {
                    if($t->getRolnames->first() != 'temsilci') unset($temsilciler[$t]);
                }

                $chat->to_user_id = array_rand($temsilciler);
                */
